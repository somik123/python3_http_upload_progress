...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        f.write(b"<hr>\n")
        f.write(b" <form id=\"form1\" ENCTYPE=\"multipart/form-data\" method=\"post\">")
        f.write(b"<input id=\"file\" name=\"file\" type=\"file\" multiple/>")
        f.write(b" <input type=\"button\" onclick=\"postFile()\" value=\"upload\"/></form>\n")
        f.write(b' <div class="progress-wrapper"> <div id="progress-bar-file" class="progress"></div> </div> \n')
        f.write(b"<hr>\n")
        f.write(b'<table>\n')
